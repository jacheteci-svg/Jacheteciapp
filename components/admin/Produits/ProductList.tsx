...
export default function ProductList({ produits }: { produits: any[] }) {
      ...
      <table className="w-full text-left border-collapse">
        <thead>
          <tr className="bg-slate-800/30">
            <th className="p-4 text-xs font-black text-slate-400 uppercase tracking-widest">Photo</th>
            <th className="p-4 text-xs font-black text-slate-400 uppercase tracking-widest">Nom</th>
            <th className="p-4 text-xs font-black text-slate-400 uppercase tracking-widest text-center">Catégorie</th>
            <th className="p-4 text-xs font-black text-slate-400 uppercase tracking-widest text-center">Prix</th>
            <th className="p-4 text-xs font-black text-slate-400 uppercase tracking-widest text-center">Stock</th>
              ...
              <tr key={p.id} className="hover:bg-slate-800/30 transition-colors group">
                ...
                <td className="p-4 text-center">
                   <div className="flex flex-col items-center gap-1">
                      <span className={`font-mono text-sm font-bold ${p.quantite <= p.seuil_alerte ? 'text-red-400' : 'text-green-400'}`}>
                        {p.quantite}
                      </span>
                      {p.quantite <= p.seuil_alerte && <span className="bg-red-400/10 text-red-400 text-[8px] font-black px-1.5 py-0.5 rounded uppercase font-mono tracking-tighter">ALERTE</span>}
                   </div>
                </td>
                <td className="p-4">
