...
export default async function Home() {
  ...
  return (
    <div className="bg-slate-50 min-h-screen text-slate-900 font-sans pb-20 overflow-x-hidden">
      <SEO />
      <Header />
      ...
      <main>
        ...
        <section className="relative bg-white overflow-hidden border-b border-slate-100">
          ...
          <div className="container mx-auto max-w-6xl px-4 py-12 md:py-24 flex flex-col items-center text-center relative z-10">
            <div className="flex items-center gap-2 bg-slate-50 border border-slate-100 px-4 py-1.5 rounded-full text-[9px] md:text-sm font-black text-brand-primary mb-5 md:mb-6 uppercase tracking-[0.2em]">
               🔥 Boutique Officielle J'achète.ci
            </div>
            <h1 className="text-3xl md:text-7xl font-black tracking-tighter text-slate-900 max-w-4xl leading-[1.1]">
            ...
            <p className="mt-6 md:mt-8 text-sm md:text-xl text-slate-500 max-w-2xl mx-auto leading-relaxed font-medium px-2">
               Commandez vos produits préférés en <span className="text-slate-900 font-bold">moins de 2 minutes</span>. 
               Qualité garantie et livraison ultra-rapide partout à Abidjan.
            </p>
            <div className="mt-8 md:mt-10 flex flex-col sm:flex-row gap-4 w-full sm:w-auto px-4 md:px-0">
               <a href="#catalogue" className="bg-brand-primary text-white px-8 py-4 md:py-5 rounded-2xl font-black text-base md:text-lg shadow-xl shadow-brand-primary/25 hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-3">
              ...
              {(produits || []).map((produit: any) => {
                ...
                return (
                  <Link 
                    key={produit.id} 
                    href={`/p/${produit.slug}`}
                    className="group bg-white rounded-[2rem] overflow-hidden border border-slate-100 hover:shadow-2xl hover:shadow-brand-primary/10 transition-all duration-700 flex flex-col relative"
                  >
                    {/* Discount Badge */}
                    {discount > 0 && (
                       <div className="absolute top-4 left-4 z-20 bg-brand-secondary text-white text-[10px] font-black px-3 py-1.5 rounded-full uppercase tracking-tighter shadow-lg shadow-brand-secondary/20">
                          -{discount}% OFF
                       </div>
                    )}
                    ...
                    {/* Content */}
                    <div className="p-6 md:p-8 flex flex-col flex-1 space-y-4 md:space-y-6">
                      <div className="space-y-2 md:space-y-3">
                         <div className="flex items-center gap-2">
                            <StockBadge quantity={produit.quantite} />
                         </div>
                        <h2 className="text-xl md:text-2xl font-black group-hover:text-brand-primary transition-colors line-clamp-2 leading-[1.1] tracking-tight">
                          {produit.nom}
                        </h2>
                      </div>
                      ...
                      <div className="pt-4 flex items-end justify-between border-t border-slate-50">
                         <div className="flex flex-col">
                            {produit.prix_barre && (
                               <span className="text-xs md:text-sm text-slate-400 line-through font-bold mb-0.5">
                        ...
                        <div className="w-12 h-12 md:w-14 md:h-14 bg-slate-900 rounded-2xl flex items-center justify-center text-white group-hover:bg-brand-primary group-hover:rotate-[360deg] transition-all duration-700 shadow-lg shadow-black/10 group-hover:shadow-brand-primary/30">
         ...
         <div className="container mx-auto max-w-4xl space-y-12">
            <div className="space-y-4">
             ...
             <div className="flex flex-col lg:flex-row gap-8 justify-center items-center">
                ...
                <div className="flex flex-row items-center gap-6 bg-white/5 px-8 py-5 rounded-3xl border border-white/5 whitespace-nowrap">
                   <a href="tel:+2250172571352" className="text-base font-black text-white flex items-center gap-3 hover:text-brand-primary transition-colors">
                      <span className="w-2.5 h-2.5 bg-brand-primary rounded-full animate-pulse"></span>
                      +225 01 72 57 13 52
                   </a>
                   <span className="h-4 w-px bg-white/10"></span>
                   <a href="tel:+2250506844901" className="text-base font-black text-white flex items-center gap-3 hover:text-brand-primary transition-colors">
